...
export default function ScanInScreen({ navigation }) {
  ...
  const handleScanned = async ({ data }) => {
    ...
    if (demo) {
      Alert.alert('Success', 'In-Time Scan Successful (Demo Mode)');
      return;
    }
    try {
      await axios.post(`${SERVER_URL}/scan-in`, { visitor_id: qr_id });
      Alert.alert('Success', 'In-Time Scan Successful');
    } catch {
      Alert.alert('Failed', 'In-Time scan failed');
    }
  };
  ...
  return (
    <ScreenContainer scroll={false}>
      ...
      <PageHeader
        ...
        subtitle="Point the camera at the visitor's QR pass to record entry."
      ...
      <View style={styles.card}>
        {!permission ? (
          ...
          <View style={styles.successBox}>
            <LinearGradient colors={gradients.successCyan} style={styles.successIcon}>
...
const styles = StyleSheet.create({
  ...
  cornerTL: { position: 'absolute', top: 14, left: 14, width: 28, height: 28, borderTopWidth: 4, borderLeftWidth: 4, borderColor: colors.accentSuccess, borderTopLeftRadius: 8 },
  cornerTR: { position: 'absolute', top: 14, right: 14, width: 28, height: 28, borderTopWidth: 4, borderRightWidth: 4, borderColor: colors.accentSuccess, borderTopRightRadius: 8 },
  cornerBL: { position: 'absolute', bottom: 14, left: 14, width: 28, height: 28, borderBottomWidth: 4, borderLeftWidth: 4, borderColor: colors.accentSuccess, borderBottomLeftRadius: 8 },
  cornerBR: { position: 'absolute', bottom: 14, right: 14, width: 28, height: 28, borderBottomWidth: 4, borderRightWidth: 4, borderColor: colors.accentSuccess, borderBottomRightRadius: 8 },
  hintRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 14 },
  greenDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: colors.accentSuccess },
